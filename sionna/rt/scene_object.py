...
class SceneObject(Object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Set velocity vector
        self.velocity = np.asarray([0., 0., 0.], dtype=self._rdtype)
    ...
    def radio_material(self, mat):
        ...
        if mat is None:
            ...
            err_msg = (
                "The material must be a material name (str) or an "
    ...
    def position(self):
        ...
        # [3]
        bbox_max = np.asarray(self._mi_shape.bbox().max, rdtype)
        # [3]
        half = float(0.5)
    ...
    def position(self, new_position):
        ...
        # [num_vertices,3]
        vertices = mi_to_np_ndarray(vertices, rdtype)
        ...
        fltn_translated_vertices = np.reshape(translated_vertices, [-1])
        fltn_translated_vertices = self._mi_scalar_t(fltn_translated_vertices)
        #
        scene_params[f"mesh-{self.name}.vertex_positions"] = fltn_translated_vertices
        scene_params.update()
        ...
        face_indices3 = mi_shape.face_indices(
            dr.arange(mi.UInt32, mi_shape.face_count())
        )
        ...
        # [n_prims*3, 3]
        vertex_coords = mi_to_np_ndarray(vertex_coords, rdtype)
        # Unflatten
        # [n_prims, vertices per triangle : 3, 3]
        vertex_coords = np.reshape(vertex_coords, [mi_shape.face_count(), 3, 3])
        # Update the tensor storing the primitive vertices
        sl = np.arange(prim_offset, prim_offset + mi_shape.face_count(), dtype=np.int_)
        ...
        # [num_wedges, 3]
        wedges_origin = np.take_along_axis(wedges_origin, wedges_ind, axis=0)
        ...
        # Updates the wedges
        wedges_ind = np.expand_dims(wedges_ind, axis=1)
    ...
    def orientation(self, new_orient):
        ...
        # Real dtype
        new_orient = np.asarray(new_orient, self._rdtype)
        ...
        # Cast to Mitsuba type to object the Mitsuba scene
        fltn_vertices = np.reshape(vertices, [-1])
        fltn_vertices = np.asarray(fltn_vertices, np.float_)
        ...
        # Cast to Tensorflow type
        # [n_prims, 3]
        normals = mi_to_np_ndarray(normals, self._rdtype)
        # Update the tensor storing the primitive vertices
        ...
        wedges_origin = np.take_along_axis(wedges_origin, wedges_ind, axis=0)
        # [num_wedges, 3]
        wedges_e_hat = np.take_along_axis(wedges_e_hat, wedges_ind, axis=0)
        # [num_wedges, 3]
        wedges_normals = np.take_along_axis(wedges_normals, wedges_ind, axis=0)
        # [num_wedges*2, 3]
        wedges_normals = np.reshape(wedges_normals, [-1, 3])
        ...
        # Cast to Tensorflow type
        # [num_wedges, 3]
        wedges_origin = mi_to_np_ndarray(wedges_origin, self._rdtype)
        # [num_wedges, 3]
        wedges_e_hat = mi_to_np_ndarray(wedges_e_hat, self._rdtype)
        # [num_wedges*2, 3]
        wedges_normals = mi_to_np_ndarray(wedges_normals, self._rdtype)
        # [num_wedges, 2, 3]
        wedges_normals = np.reshape(wedges_normals, [-1, 2, 3])

        # Updates the wedges
        wedges_ind = np.expand_dims(wedges_ind, axis=1)
        solver_paths.wedges_origin.scatter_nd_update(wedges_ind, wedges_origin)
        solver_paths.wedges_e_hat.scatter_nd_update(wedges_ind, wedges_e_hat)
        solver_paths.wedges_normals.scatter_nd_update(wedges_ind, wedges_normals)
